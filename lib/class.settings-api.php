...
if ( !class_exists('PP_Settings_API')) {
    ...
    class PP_Settings_API {
        ...
        public function register_menu() {
            ...
            if ($this->settings_menu['multisite_only']){
                ...
                add_menu_page($this->settings_menu['title'], $this->settings_menu['title'], $role, $this->settings_menu['name'], array(&$this, 'render_option_page'), HMW_URL . '/img/hmwp-logo-t.png', 60);
            ...
            if ($can_deactive && is_admin() && isset($_POST['action']) && $_POST['action'] == 'update' && isset($_POST['option_page']) && $_POST['option_page'] == $this->settings_menu['name']) {
                ...
                if (isset($_POST['import_field']) && $_POST['import_field'] && check_admin_referer($this->settings_menu['name'] . '-options')) {
                    ...
                    if (isset($new_settings['li']) && !trim($new_settings['li']) && $_POST[$this->settings_menu['name']]['li'])
                        $new_settings['li'] = sanitize_text_field($_POST[$this->settings_menu['name']]['li']);
        ...
        public function plugin_actions_links($links) {
            if ($this->settings_menu['action_link']) {
                $setting_link = '<a href="' . admin_url("admin.php?page=" . $this->settings_menu['name']) . '" >' . $this->settings_menu['action_link'] . '</a>';
...
//       }
        ...
        function callback_text($args) {
            ...
            $disable_option = isset($args['disable']) && $args['disable'] == '1' ? ' disabled="disabled"' : '';
            $html = '';
            if ($disable_option) {
                $html .= '<div class="disable_options">';
            }
            $html .= sprintf('<input type="text" class="regular-text %1$s" id="%4$s" name="%2$s[%4$s]" value="%5$s"%6$s/>', $args['class'], $this->settings_menu['name'], $args['section'], $args['id'], $value, $disable_option);
        ...
        function callback_number($args) {
            ...
            $html = sprintf('<input type="text" class="regular-number %1$s" id="%4$s" name="%2$s[%4$s]" value="%5$s" %6$s style="width:50px;"/>', $args['class'], $this->settings_menu['name'], $args['section'], $args['id'], $value, $disable);
            $html .= sprintf('<span class="description"> %s</span>', $args['desc']);
        ...
        function callback_checkbox($args) {
            ...
            $html .= sprintf('<input type="checkbox" class="checkbox %1$s" id="%3$s" name="%2$s[%3$s]" value="on"%4$s%5$s/>', $args['class'], $this->settings_menu['name'], $args['id'], checked('on', $value, false), $disable_option);
        ...
        function callback_radio($args) {
            $value = $this->get_option($args['id'], $this->settings_menu['name']);
            $disable_option = isset($args['disable']) && $args['disable'] != '' ? explode(',', $args['disable']) : array();
            $html = '';
            foreach ($args['options'] as $key => $label) {
                $disabled = $disabled_class = '';
                if (in_array($key, $disable_option)) {
                    $disabled = ' disabled="disabled"';
                    $disabled_class = 'disabled_option';
                    $html .= '<label class="' . $disabled_class . '">';
                }
                $html .= sprintf('<input type="radio" class="radio %1$s" id="%3$s_%4$s" name="%2$s[%3$s]" value="%4$s"%5$s%6$s />', $args['class'], $this->settings_menu['name'], $args['id'], $key, checked($value, $key, false), $disabled);
                $html .= sprintf('<label for="%2$s_%4$s"> %3$s</label><br>', $this->settings_menu['name'], $args['id'], $label, $key);
                if (in_array($key, $disable_option)) {
                    $html .= '</label>';
                }
        ...
        function callback_select($args) {
            ...
            $disable_option = isset($args['disable']) && $args['disable'] != '' ? explode(',', $args['disable']) : array();
            $html = '';
            if (in_array('all', $disable_option)) {
                $html .= '<div class="disable_options">';
            }
            ...
            foreach ($args['options'] as $key => $label) {
                ...
                $html .= sprintf('<option value="%s"%s%s>%s</option>', $key, selected($value, $key, false), $disabled, $label);
            }
            $html .= sprintf('</select>');
            $html .= sprintf('<span class="description"> %s</span>', $args['desc']);
            if (in_array('all', $disable_option)) {
        ...
        function callback_rolelist($args) {
            ...
            foreach ($args['options'] as $key => $label) {
                ...
                $html .= sprintf('<input type="checkbox" class="checkbox user_roles_checkbox %1$s" id="%3$s_%4$s" name="%2$s[%3$s][%4$s]" value="%4$s"%5$s disabled=disabled/>', $args['class'], $this->settings_menu['name'], $args['id'], $key, checked($checked, $key, false));
                $html .= sprintf('<label for="%2$s_%4$s"> %3$s</label><br>', $this->settings_menu['name'], $args['id'], $label, $key);
            }
            $html .= sprintf('<span class="description"> %s</label>', $args['desc']);
            $html .= '</div>';
        ...
        function callback_textarea($args) {
            ...
            $html = sprintf('<textarea rows="5" cols="65" class="regular-text %1$s" id="%3$s" name="%2$s[%3$s]" %5$s>%4$s</textarea>', $args['class'], $this->settings_menu['name'], $args['id'], $value, $disable);
        ...
        function callback_export($args) {
            echo '<a href="#" class="button">' . __('Export Current Settings', $this->settings_menu['name']) . '</a>';
        ...
        function callback_import($args) {
            ...
            if ($args['options']) {
                ...
                foreach ($args['options'] as $key => $settings_value) {
                    $disabled = '';
                    if (in_array($settings_value, $disable_option)) {
                        $disabled = ' disabled="disabled"';
                    }
                    $html .= sprintf('<option value="%s"%s>%s</option>', esc_textarea(stripslashes($settings_value)), $disabled, ucfirst($key));
                }
        ...
        function script() {
            ...
            <script>
                jQuery(document).ready(function ($) {
                    ...
                    $('.group').hide();
                    $('#replaces,#source,#ids').click(function () {
                        window.open('https://codecanyon.net/item/hide-my-wp-amazing-security-plugin-for-wordpress/4177158?s_rank=1', '_blank');
                    });
                    $('tr.upgrade_to_pro.disable_option_class label.disabled_option, tr.upgrade_to_pro.disable_all_option div.disable_options').click(function (event) {
                    ...
                    $('#import_options').change(function (e) {
                        if ($(this).val() == 'pro') {
                            alert('To use this import option you have upgrade to PRO');
                            return false;
                        }
